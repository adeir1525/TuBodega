		...
		$.ajax({
			...
			success : function(response){
				...
				if (resp.status == 202) {
					...
					if (productList) {
						$.each(resp.message.products, function(index, value){
							...
							productHTML += '<tr>'+
								              ...
								              '<td><img width="60" height="60" src="../imagenes/'+value.product_image+'"></td>'+
					...
					var catSelectHTML = '<option value="">Seleccionar categoria</option>';
					...
					var brandSelectHTML = '<option value="">Seleccionar marca</option>';
...
$(".add-product").on("click", function(){
	...
	*/
	...
	$(document.body).on('click', '.edit-product', function(){
		...
		$("input[name='e_product_image']").siblings("img").attr("src", "../imagenes/"+product.product_image);
	...
	$(document.body).on('click', '.delete-product', function(){
		...
		if (confirm("¿Estas de seguro de eliminar este ITEM?")) {
			$.ajax({
