...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/marketline/imagenes/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Producto añadido con éxito ..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Error al ejecutar la consulta'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Error al cargar la imagen'];
				}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Producto eliminado de existentes'];
			}else{
				return ['status'=> 202, 'message'=> 'Error al ejecutar la consulta'];
			}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'ID de categoría no válida'];
		}
